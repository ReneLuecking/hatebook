 ...
 */
class Version20180131082905 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, picture_id, first_name, last_name, email, birthday FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER NOT NULL, picture_id INTEGER DEFAULT NULL, first_name VARCHAR(255) NOT NULL COLLATE BINARY, last_name VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(255) NOT NULL COLLATE BINARY, birthday DATE NOT NULL, password VARCHAR(255) NOT NULL, PRIMARY KEY(id), CONSTRAINT FK_8D93D649EE45BDBF FOREIGN KEY (picture_id) REFERENCES file (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO user (id, picture_id, first_name, last_name, email, birthday) SELECT id, picture_id, first_name, last_name, email, birthday FROM __temp__user');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, picture_id, first_name, last_name, email, birthday FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER NOT NULL, picture_id INTEGER DEFAULT NULL, first_name VARCHAR(255) NOT NULL, last_name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, birthday DATE NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO user (id, picture_id, first_name, last_name, email, birthday) SELECT id, picture_id, first_name, last_name, email, birthday FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649EE45BDBF ON user (picture_id)');
    }
